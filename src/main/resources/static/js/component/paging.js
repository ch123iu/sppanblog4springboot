		...
		$.ajax({
			...
			success: function(result, status, xhr) {
				if(result) {
					...
					if(_config.paged) {
						if(result.totalElements === null || result.totalElements === 0) {
							throwError('Paging Error:请返回数据总数！');
							return;
						}
						var _pageConfig = _config.pageConfig;
						var pageSize = _pageConfig.pageSize;
						var pages = result.totalElements % pageSize == 0 ?
							(result.totalElements / pageSize) : (result.totalElements / pageSize + 1);
